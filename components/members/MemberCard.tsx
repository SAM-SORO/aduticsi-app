...
export function MemberCard({ member }: { member: MemberCardData }) {
  ...
  return (
    <article className="group relative flex flex-col rounded-2xl border border-slate-200 bg-white p-5 transition-colors duration-200 hover:border-slate-300 focus-within:border-slate-400">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="truncate text-base font-semibold text-slate-900" title={fullName}>
            {/* Lien etire : rend toute la carte cliquable sans imbriquer de liens. */}
            <Link href={href} className="outline-none after:absolute after:inset-0 after:rounded-2xl focus-visible:after:ring-2 focus-visible:after:ring-[var(--aduti-primary)]">
              {fullName}
            </Link>
          </h3>
          {role ? (
            <p
              className={`mt-0.5 line-clamp-2 text-sm ${member.poste ? "font-medium text-[var(--aduti-primary)]" : "text-slate-500"}`}
              title={role}
            >
              {role}
            </p>
          ) : (
            <p className="mt-0.5 text-sm text-slate-400">Membre</p>
          )}
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
        ...
        <span className="flex items-center gap-1 text-sm font-medium text-slate-400 transition-colors group-hover:text-[var(--aduti-primary)]">
          Voir le profil
